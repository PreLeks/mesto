export const validationProps = {
  formSelector: '.popup__form',
  inputSelector: '.popup__item',
  submitButtonSelector: '.popup__button-submit',
  inactiveButtonClass: 'popup__button-submit_disabled',
  inputErrorClass: 'popup__item_error',
  errorClass: 'popup__item-error_active',
};

export const profileAbout = ('.profile__about');
export const imageInPopup = ('.popup__image');
export const deleteCard = ('.popup_delete-card');

export const popupAvatar = ('.popup__avatar');
export const profileAvatar = ('.profile__image');
export const formAvatar = document.querySelector('.popup__input_container-avatar');
export const editAvatarButton = document.querySelector('.profile__button-avatar');

export const nameInput = document.querySelector('.popup__item_input-name');
export const nameProfile = ('.profile__name');

export const descriptionInput = document.querySelector('.popup__item_input-about');

export const addButton = document.querySelector('.profile__button-add');
export const editButton = document.querySelector('.profile__button-edit');

export const addPopup = ('.popup__add');
export const editPopup = ('.popup__edit');

export const formPlace = document.querySelector('.popup__input_container-add');
export const formAbout = document.querySelector('.popup__input_container-edit');

export const placeTemplate = ('.place-template');
export const placeCatalogue = ('.places__catalogue');

export const submitButtonPlace = document.querySelector('.popup__button_type_place');
export const submitButtonAbout = document.querySelector('.popup__button-submit-edit');
export const submitButtonAvatar = document.querySelector('.popup__button-submit-avatar');